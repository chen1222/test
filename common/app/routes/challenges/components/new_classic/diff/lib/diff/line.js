...
lineDiff.tokenize = function (value) {
  var retLines = [],
      origLines = [],
      linesAndNewlines = value.replace(/\"/g, '\'').split(/(\n)/);
  ...
  for (var i = 0; i < linesAndNewlines.length; i++) {
    ...
    if (i % 2 && !this.options.newlineIsToken) {
      ...
      origLines[origLines.length - 1] += line;
    } else {
      if(line !== '\n') {
        origLines.push(line);
        ...
        console.log('diff', 'empty return');
      }
    }
  }
  retLines = retLines.map(x => {
    if(x.length == 1)
      return x;
    else
      return x.replace('\n', '');
  });
  // console.log(retLines, 'diff', 'aaa');
  return {
    retLines: retLines,
    origLines: origLines
  };
