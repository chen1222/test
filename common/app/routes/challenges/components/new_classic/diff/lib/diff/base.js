...
Diff.prototype = { /*istanbul ignore start*/
  /*istanbul ignore end*/diff: function diff(oldString, newString) {
    ...
    var newToken = this.tokenize(newString);
    var oldToken = this.tokenize(oldString);

    oldString = this.removeEmpty(oldToken.retLines);
    newString = this.removeEmpty(newToken.retLines);

    var oldOrigString = this.removeEmpty(oldToken.origLines);
    var newOrigString = this.removeEmpty(newToken.origLines);
    ...
    if (bestPath[0].newPos + 1 >= newLen && oldPos + 1 >= oldLen) {
      ...
      return done([{ value: this.join(newOrigString), count: newString.length }]);
    }
    ...
    function execEditLength() {
      for (var diagonalPath = -1 * editLength; diagonalPath <= editLength; diagonalPath += 2) {
        ...
        if (basePath.newPos + 1 >= newLen && _oldPos + 1 >= oldLen) {
          return done(buildValues(self, basePath.components, newOrigString, oldOrigString, self.useLongestToken));
        } else {
...
function buildValues(diff, components, newString, oldString, useLongestToken) {
  ...
  for (; componentPos < componentLen; componentPos++) {
    ...
    if (!component.removed) {
      ...
      // Common case
      if (!component.added) {
        component.oldValue = diff.join(oldString.slice(oldPos, oldPos + component.count));
